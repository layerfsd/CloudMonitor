...
bool StartInteractiveProcess(LPSTR cmd, LPSTR cmdDir)
{
	...
	si.cb = sizeof(si);
	si.lpDesktop = "winsta0\\default";  // Use the default desktop for GUIs
	...
	if (::WTSQueryUserToken(sessionId, &dummy)) {
		...
		if (!::CreateProcessAsUserA(token, NULL, cmd, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, cmdDir, &si, &pi)) {  // The "new console" is necessary. Otherwise the process can hang our main process
			::CloseHandle(token);
			return false;
		}
		::CloseHandle(token);
	...
	else if (!::CreateProcessA(NULL, cmd, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, cmdDir, &si, &pi))  // The "new console" is necessary. Otherwise the process can hang our main process
		return false;
...
void InstallService();
bool StartInteractiveProcess(const char* cmd, LPCTSTR cmdDir);
