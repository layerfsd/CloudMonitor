...
# 把hashlist.txt 文件保存在‘根’下
hash_file = (r'Release/DAta/hashlist.txt', 'hashlist.txt')

def make_zip(version):
    zip_name = version + ".zip"
    zipf = zipfile.ZipFile(zip_name, 'w')
    keep_list = path_list
    keep_list.append(hash_file)
    for local_path, arcname in path_list:
        arcname = os.path.join(version, arcname)
        # print(local_path, arcname)
